...
public class CouponRoute {
    ...
    private final CouponRoutesProperties properties;
                    ...
                    operation = @Operation(operationId = "coupon", tags = {"Coupon"},
                            ...
                            responses = {
                                    @ApiResponse(responseCode = "200", content = @Content(
                                            schema = @Schema(implementation = CouponResponse.class, description = "Get the max items from an item list and coupon value", example = "<br>{\n" +
                                                    "\"item_ids\": [\"MLA844702264\", \"MLA885218459\", \"MLA844702264\"],\n" +
                                                    "\"amount\": 2080 \n" +
                                                    "}"))),
                                    @ApiResponse(responseCode = "400",
                                            content = @Content(
                                                    schema = @Schema(implementation = Error.class)))})),
            @RouterOperation(method = RequestMethod.GET, path = "/api/v1/coupon/stats",
                    operation = @Operation(operationId = "couponStats", tags = {"Coupon Stats"},
                            parameters = {
    ...
    public RouterFunction<ServerResponse> routes(CouponHandler handler) {
        return nest(path(properties.getBasePath()).and(accept(APPLICATION_JSON)),
                route(POST(properties.getRouteCoupon()), handler::maxItemsCoupon)
                .andRoute(GET(properties.getCouponStats()), handler::couponStats));
    }

    private String getPath(String variable) {
        String transactionPath = properties.getBasePath()
                .concat(properties.getRouteCoupon());
